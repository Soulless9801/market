...
import { buildFeatures, createSideResolver, createSideNormalizer, SIDE_ACTIONS } from "@/simulation";
...
export function buildDefaultAgents(seed: number, referencePrice = 100): TraderAgent[] {
	return [
		...
		new RetailTraderAgent("retail-4", {
			referencePrice,
			spread: 2,
			quantity: 8,
			seed: seed + 2,
		...
		new RetailTraderAgent("retail-5", {
			referencePrice,
			spread: 2,
			quantity: 8,
			seed: seed + 3,
		...
		new RetailTraderAgent("retail-6", {
			referencePrice,
			spread: 2,
			quantity: 8,
			seed: seed + 3,
		...
		new RetailTraderAgent("retail-7", {
			referencePrice,
			spread: 2,
			quantity: 8,
			seed: seed + 3,
